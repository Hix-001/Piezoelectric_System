...
import {
  ...
  MeshTransmissionMaterial, Float, Sparkles,
...
const CAM = [
  { pos: [0, 2.8, 8.5],   look: [0, 0, 0]          }, // hero
  { pos: [0.5, 3.8, 6.5], look: [0, 0.5, 0]         }, // exploded
  { pos: [2.6, 0.6, 3.2], look: [2.0, -0.15, 0]     }, // circuit
  { pos: [0.8, 2.2, 6.8], look: [1.0, -0.1, 0]      }, // storage
  { pos: [0, 5.5, 13],    look: [0, 0, 0]            }, // conclusion
]
...
// ─────────────────────────────────────────────────────────────────────────────
// HELPER: smooth lerp ref
// ─────────────────────────────────────────────────────────────────────────────
function useLerpRef(initial) {
...
// ─────────────────────────────────────────────────────────────────────────────
// WIRE — thin cylinder connecting PCB to battery
// ─────────────────────────────────────────────────────────────────────────────
function Wire({ from, to, color = C.purple }) {
  ...
  const curve = useMemo(() => new THREE.QuadraticBezierCurve3(...points), [points])
...
function SparkParticles({ active, count }) {
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const t = data.ts[i]
      const arcY = Math.sin(t * Math.PI) * data.arcs[i]
      pos[i*3]   = THREE.MathUtils.lerp(data.starts[i*3],   data.ends[i*3],   t)
      pos[i*3+1] = THREE.MathUtils.lerp(data.starts[i*3+1], data.ends[i*3+1], t) + arcY
...
function EnergyMat({ scrollT, particleCount }) {
  ...
  const camPos   = useLerpRef(new THREE.Vector3(...CAM[0].pos))
  const camLook  = useLerpRef(new THREE.Vector3(...CAM[0].look))

  useFrame((state, delta) => {
    const sec = Math.min(4, Math.floor(scrollT * 5))
    const lT  = (scrollT - sec * 0.2) / 0.2  // local t within section
  ...
  return (
    <group ref={groupRef}>
      ...
      {[-0.6, 0, 0.6].map((z, i) => (
        <mesh key={i} position={[0, -0.23, z]}>
          ...
          <meshStandardMaterial
            color={C.blue}
            emissive={C.blue}
            emissiveIntensity={sensorActive ? 1.5 : 0.3}
          />
        </mesh>
      ))}

      {/* ── Foam top layer (transmissive glass look) ── */}
      ...
      {SENSOR_POS.map((pos, i) => (
        <Sensor key={i} position={pos} active={sensorActive} />
      ))}
      ...
      {BATTERY_POS.map((pos, i) => (
        <group key={i} position={pos}>
          ...
          <pointLight
            color={C.purple}
            intensity={storageActive || conclusionSec ? 2.5 : 0.2}
            distance={2}
            decay={2}
          />
        </group>
      ...
      {/* ── Under-mat ambient light ── */}
      <pointLight
        position={[0, -0.4, 0]}
        color={C.blue}
        intensity={sensorActive ? 3.5 : 0.4}
        distance={6}
        decay={2}
      />

      {/* ── Conclusion wide glow ── */}
      {conclusionSec && (
        <pointLight
          position={[0, 2, 0]}
          color={C.purple}
...
function PostFX({ scrollT }) {
  ...
  return (
    <EffectComposer multisampling={0}>
      <Bloom
        intensity={isConclusion ? 2.5 : 1.2}
...
// ─────────────────────────────────────────────────────────────────────────────
// SCENE INNER — everything inside the Canvas
// ─────────────────────────────────────────────────────────────────────────────
function SceneInner({ scrollT }) {
  ...
  return (
    <>
      <PerformanceMonitor
        onDecline={() => setParticleCount(60)}
        onIncline={() => setParticleCount(180)}
        flipflops={3}
        factor={0.5}
        iterations={5}
      />
...
function TextOverlay({ scrollT }) {
  ...
  return (
    <div className="overlay" aria-hidden="false">
      ...
      {/* Circuit scanlines — only in section 2 */}
      {sec === 2 && (
...
export default function App() {
  ...
  useMotionValueEvent(scrollYProgress, 'change', v => setScrollT(v))

  // Simulate load complete after short delay
  useEffect(() => {
